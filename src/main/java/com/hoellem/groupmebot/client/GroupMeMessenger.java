...
import com.hoellem.groupmebot.model.groupme.FindAllGroupsResponse;
import com.hoellem.groupmebot.model.groupme.FindGroupDetailsResponse;
import com.hoellem.groupmebot.model.groupme.BotResponse;
...
public class GroupMeMessenger
{
  ...
  public void sendGroupMeMessage(String text)
  {
    if (text != null)
    {
      BotResponse botResponse = new BotResponse(groupMeConfig.getBotId(), text);
      HttpEntity<BotResponse> groupMePost = new HttpEntity<>(botResponse, headers);
      restTemplate.exchange(baseUrl + postPath, HttpMethod.POST, groupMePost, String.class);
      logger.info("Posted " + botResponse);
    }
  }
